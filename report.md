...
|---|---|---|
| Loss | 0.0007 | 0.1980 |
| Accuracy | 100.00% | 94.54% |

Training accuracy reaches 100% by ~epoch 20 while validation accuracy
plateaus around 94–95%, with validation loss flattening (and very slightly
...
| Metric | Value |
|---|---|
| **Test Accuracy** | **94.98%** (1,496 / 1,575 correct) |
| Macro Precision | 0.9505 |
| Macro Recall | 0.9498 |
| Macro F1 | 0.9497 |

Full per-class precision/recall/F1 is saved in `outputs/test_metrics.txt`. The
weakest classes were **1** (F1 = 0.828), **5** (F1 = 0.867), and **B** /
**I** (F1 ≈ 0.876–0.899), all of which are discussed below.
...
character pairs: **J/2**, **N/X**, **E/6**, **2/7**, **9/5**, **S/5**, **W/N**.
...
visually ambiguous at low resolution (28×28) — they are
...
├── data/                            # generated by main.py (not tracked in git)
│   ├── raw_dataset.npz
│   └── splits.npz
└── outputs/
    ├── weights/model.npz
...
10,500 images. The final model reaches **94.98% test accuracy** with a macro
F1 of **0.9497**, and its small number of errors are concentrated in
